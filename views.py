...
def setscanfile(request, scanfile):
    ...
    return render(
        request,
        "nmapreport/nmap_hostdetails.html",
...
def details(request, address):
        ...
        if str(saddress) == address:
            ...
            if scanmd5 in labelhost:
                if addressmd5 in labelhost[scanmd5]:
                    ...
                    labelToMargin(labelhost[scanmd5][addressmd5])
                    (
    ...
    r["table"] = ""
    _notesout, notesb64, _removenotes = "", "", ""
...
def index(request, filterservice="", filterportid=""):
    ...
    r["webmapver"] = (
        "WebMap "
        + gitcmd.read()
        + "<br>This project is currently a beta, please <b>DO NOT</b> expose WebMap to internet.<br>This version is <b>NOT</b> production ready."
    )

    if "scanfile" in request.session:
        oo = xmltodict.parse(
            open("/opt/xml/" + request.session["scanfile"], "r").read()
        )
        r["out2"] = json.dumps(oo["nmaprun"], indent=4)
        o = json.loads(r["out2"])
    else:
