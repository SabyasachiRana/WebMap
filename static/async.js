...
$(document).ready(function () {
  ...
  $.get("https://api.github.com/repos/Rev3rseSecurity/WebMap").done(
    function (d) {
...
function newscan() {
  $("#modaltitle").html(
    '<i class="material-icons">wifi_tethering</i> New Nmap Scan',
  );
  $("#modalbody").html(
  ...
  $("#modalfooter").html(
    '<button onclick="javascript:startscan();" class="btn green">Start</button>',
  );
  $("#modal1").modal("open");
  $("select").formSelect();
}
...
function checkCVE() {
  ...
  $.post("/report/api/getcve/", {
    ...
    console.log(d);
    $("#modalbody").html(
      "Done. Please, reload this page by clicking on Reload button.",
    );
    $("#modalfooter").html(
      '<button class="btn blue" onclick="javascript:location.reload();">Reload</button>',
    );
  });
...
function saveNotes() {
  ...
  $.post("/report/api/savenotes/", {
    ...
    if (typeof d["ok"] !== "undefined" && d["ok"] == "notes saved") {
      $("#modalbody").html(
        '<span class="green-text">Notes successfully saved!</span><br> Now this page needs a reload. Please, click on the &quot;Reload&quot; button.',
      );
      $("#modalfooter").html(
        '<button class="btn blue" onclick="javascript:location.reload();">Reload</button>',
      );
  ...
  if (/^[a-f0-9]{32,32}$/.test(hashstr)) {
    ...
    $("#modalbody").html(
      "In the text area below, you can insert notes that will appear on the PDF report. " +
        ...
        '<textarea id="notes" style="min-height:160px;border-radius:4px;border:solid #ccc 1px;padding:10px;font-family:monospace;">' +
        $("<span/>").text(savednotes).html() +
        "</textarea>" +
        '<input type="hidden" id="hashstr" value="' +
        hashstr +
        '" /><br><br>' +
        "<b>Tips:</b><br>" +
    ...
    $("#modalfooter").html(
      '<button class="modal-close waves-effect waves-green btn grey">Close</button> <button onclick="javascript:saveNotes();" class="waves-effect waves-green btn green white-text">Save</button>',
    );
    $("#modal1").modal("open");
  }
}
...
function apiPortDetails(address, portid) {
  $.get("/report/api/" + address + "/" + portid + "/").done(function (data) {
    ...
    $("#modaltitle").html(
      'Port Details: <span class="blue-text">' +
        $("<span/>").text(data["@protocol"]).html().toUpperCase() +
        " / " +
        $("<span/>").text(data["@portid"]).html() +
        "</span>",
    );
    ...
    if (typeof data["script"] !== "undefined") {
      if (
        ...
        typeof data["script"]["@id"] === "undefined"
      ) {
        for (sid in data["script"]) {
          if (
            typeof ingorescriptid[data["script"][sid]["@id"]] !== "undefined"
          ) {
            continue;
          }
          tbody +=
            '<tr><td class="orange-text">' +
            $("<span/>").text(data["script"][sid]["@id"]).html() +
            '</td><td style="font-family:monospace;font-size:12px;">' +
            $("<span/>").text(data["script"][sid]["@output"]).html() +
            "</td></tr>";
        }
      } else {
        if (typeof ingorescriptid[data["script"]["@id"]] === "undefined") {
          tbody +=
            '<tr><td class="orange-text">' +
            $("<span/>").text(data["script"]["@id"]).html() +
            '</td><td style="font-family:monospace;font-size:12px;">' +
            $("<span/>").text(data["script"]["@output"]).html() +
            "</td></tr>";
        }
      }
    } else {
    ...
    $("#modalbody").html(
      '<table class="table"><thead><th style="min-width:200px;">Script ID</th><th>Output</th></thead><tbody>' +
        tbody +
        "</tbody></table>",
    );
    $("#modalbody").append(
      "<br><b>Raw Output:</b><br><pre>" +
        $("<span/>")
...
function removeLabel(type, hashstr, i) {
  $.get("/report/api/rmlabel/" + type + "/" + hashstr + "/").done(
    function (data) {
...
function setLabel(type, label, hashstr, i) {
  $.get(
    "/report/api/setlabel/" + type + "/" + label + "/" + hashstr + "/",
  ).done(function (data) {
    console.log(data);
    var res = data;
    var color = "grey";
